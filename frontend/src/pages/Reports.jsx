  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        {TABS.map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-2 rounded-xl capitalize font-semibold text-sm transition ${
              tab === t
        ...
        <div className="ml-auto flex gap-2 items-center flex-wrap">
          {tab === 'daily' && (
            <input type="date"
              className="border dark:border-gray-600 rounded-xl px-3 py-2 text-sm bg-white dark:bg-gray-800 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none"
          ...
          {tab === 'range' && (
            <>
              ...
              <button onClick={load} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl text-sm font-semibold transition">Apply</button>
            </>
          )}
        </div>
      </div>
        ...
        <div className="space-y-5">
          ...
          {paymentPie.length > 0 && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-5">
                <h3 className="font-bold text-gray-900 dark:text-white text-sm mb-4">Payment Breakdown</h3>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-5">
                <h3 className="font-bold text-gray-900 dark:text-white text-sm mb-4">Sales by Method</h3>
                <ResponsiveContainer width="100%" height={200}>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b dark:border-gray-700">
              <h3 className="font-bold text-gray-900 dark:text-white text-sm">Transactions ({report.sales?.length})</h3>
            </div>
                ...
                <tbody className="divide-y dark:divide-gray-700">
                  ...
                  {report.sales?.map(s => (
                    <tr key={s.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition">
                      ...
                      <td className="px-4 py-3 dark:text-gray-300">{s.customer?.name || <span className="text-gray-400 italic">Walk-in</span>}</td>
                      <td className="px-4 py-3 dark:text-gray-300">{s.items?.length}</td>
      ...
      {!loading && inventory && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-5">
            <h3 className="font-bold text-gray-900 dark:text-white text-sm mb-4">All Products ({inventory.allProducts?.length})</h3>
            <div className="space-y-2 max-h-96 overflow-auto">
              {inventory.allProducts?.map(p => (
                <div key={p.id} className="flex justify-between items-center py-2 border-b dark:border-gray-700 last:border-0">
                  <div>
                    <p className="font-medium text-sm dark:text-white">{p.name}</p>
                    <p className="text-xs text-gray-400">{p.category}</p>
                  </div>
                  <Badge label={`${p.quantity} in stock`} color={p.quantity <= p.lowStockThreshold ? 'red' : 'green'} />
                </div>
              ))}
            </div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-5">
            <h3 className="font-bold text-red-600 text-sm mb-4">Low Stock ({inventory.lowStock?.length})</h3>
            {inventory.lowStock?.length === 0 ? (
              <p className="text-gray-400 text-sm text-center py-8">All products are well stocked</p>
            ) : (
              <div className="space-y-2">
                {inventory.lowStock?.map(p => (
                  <div key={p.id} className="flex justify-between items-center bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/20 rounded-xl px-4 py-3">
                    <div>
                      <p className="font-semibold text-sm dark:text-white">{p.name}</p>
                      <p className="text-xs text-gray-500">{p.supplier || 'No supplier'}</p>
                    </div>
                    <span className="text-red-600 font-black">{p.quantity} left</span>
                  </div>
                ))}
              </div>
            )}
